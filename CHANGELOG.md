...
- **`lefthook.yml`** — replaces `.pre-commit-config.yaml`. Parallel pre-commit gates (shellcheck, shfmt, actionlint, zizmor, bats syntax) + commit-msg Conventional Commits validator. Lefthook is the canonical hook runner across yolo-labz/* per release-engineering canon. Closes spec 024 PR-H wave 3.
- **`cliff.toml`** — git-cliff config emitting Keep-a-Changelog 1.1.0 sections (Added / Changed / Deprecated / Removed / Fixed / Security) from Conventional Commits. Skips ci/build/test/chore(deps)/chore(release) noise. Future releases will use `git cliff` to regenerate this file; hand-editing stops being the supported flow.
- **`.github/workflows/shellcheck-sarif.yml`** — closes the static-analysis gap: CodeQL has no Bash support, so shellcheck runs on every push/PR, output is converted to SARIF via `psastras/sarif-rs#shellcheck-sarif` v0.8.0, and uploaded to GitHub's code-scanning dashboard via `github/codeql-action/upload-sarif`. Findings now surface in the Security tab alongside Scorecard / OSV-Scanner / CodeQL. Workflow runs with `permissions: {}` deny-all + `step-security/harden-runner@v2.19.4 egress-policy: audit`.
- **`.claude-plugin/marketplace.json`** — required by Claude Code v2.1.108+ to register the repository as a local or remote marketplace. Both documented install paths (`/plugin marketplace add yolo-labz/claude-mac-chrome` and the local-clone variant) failed with `Marketplace file not found` because Claude Code does not auto-synthesize the manifest from `plugin.json`. Closes #36 (gap 1).
- **Bash 4.0+ preflight in `chrome-lib.sh`** — fail-fast version check at the top of the library with an actionable diagnostic pointing at `brew install bash` and the PATH export needed for Apple Silicon and Intel. Without this, the script silently failed deep inside helpers with `mapfile: command not found` on macOS's stock `/bin/bash` 3.2.57. Closes #36 (gap 2).
- **Bash 4.0+ requirement** documented in README under a new `Requirements` section, with copy-paste install snippets for both Apple Silicon and Intel macOS.
...
- **`chrome_js_async` primitive** — runs JS whose return value is a Promise (or any value) and awaits the settlement via the title-sentinel pattern. A per-call random sentinel (`__cmc_<16 hex>`) prefixes the resolved/rejected payload, which is JSON-stringified into `document.title`; the host polls the tab title at 100 ms cadence until the sentinel appears, parses the payload, and restores the original title from a `window["__cmc_orig_<sentinel>"]` stash. Default timeout 10 s (configurable 1 – 600 s) returns a `{"ok":false,"error":"chrome_js_async_timeout"}` envelope. New `chrome_tab_title` helper + `js_async` / `tab_title` CLI verbs. 14 bats + happy-dom wrapper probe cover sync return, Promise resolve/reject, synchronous throw, undefined → null coercion, title stash, and arg validation.
- **TOCTOU element fingerprint** (NFR-SR-V2-TOCTOU) — `_chrome_safety_check_js` now snapshots `{tag, id, outerHTML_hash, rect}` into `envelope.element_fingerprint` (FNV-1a 32-bit hash over the first 5000 chars of `outerHTML`). `chrome_click` extracts it via jq, injects as `_EXPECTED_FP` into the dispatch JS, and the dispatch layer recomputes + compares after `el.isConnected`. Any drift on tag / id / hash or rect beyond 4 px jitter returns `{ok:false, error:"toctou_drift", drift:[...]}`, which the bash layer routes to a `SAFETY_BLOCK` envelope with `reason: "toctou_drift"` and audits as `blocked`. Closes the race where an attacker mutates the DOM after safety-pass but before click dispatch. Happy-dom probe + 8 structural bats cover emit, injection, drift detection, and backward-compat null-fingerprint bypass.
- **Audit log rotation + append-only enforcement** (NFR-SR-V2-16, NFR-SR-V2-AUDIT-ROT) — `_chrome_audit_rotate_if_needed` triggers single-slot rotation (`action-audit.jsonl` → `action-audit.jsonl.1`) when the log crosses 10 MiB (override via `CHROME_AUDIT_ROTATE_BYTES`). A mkdir-based mutex (portable on macOS without `flock(1)`) serializes concurrent rotations; `chflags uappnd` marks both current and rotated files append-only on Darwin (override via `CHROME_AUDIT_APPEND_ONLY=0`). Truncating writes fail under the flag, but append redirects (`>>`) continue to work, so audit integrity holds even under compromise. 12 new bats cover size boundary, contention, JSONL validity across rotation, single-slot overwrite, and chflags persistence.

### Removed

- **`.pre-commit-config.yaml`** — migrated to `lefthook.yml`. Contributors should run `lefthook install` instead of `pre-commit install`.

## [1.1.1] — 2026-04-12 — SBOM fix

### Fixed

- `syft` installed by `anchore/sbom-action/download-syft@v0.24.0` does not support the `cyclonedx-json@1.7` version suffix. Dropped the `@1.7` suffix — syft produces the highest CycloneDX version it supports. v1.1.0 release was published without SBOMs; v1.1.1 includes both `sbom.cdx.json` and `sbom.spdx.json`.
